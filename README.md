# 实时同声传译 · 中 ⇄ English / 日本語

面对面会谈用的双向同声传译工具。你说中文，对方立刻听到译文语音并看到译文字幕；
对方说英文或日语，你立刻听到中文语音并看到中文字幕。
...
| **OpenAI** | `gpt-realtime-translate` | 自动识别语种，需代理与境外信用卡，**不支持术语库、不支持中日** |
...
| 操作 | 说明 |
|---|---|
| **中英自动 / 中日自动** | 切换语言对。方向全自动识别，无需手动指定谁在说话 |
| **空格键** | 按住临时静音麦克风，松开恢复 |
| **⛶ 全屏** | 隐藏工具栏、字号放大，方便转屏给客户看 |
| **⚙ 设置** | 后端、术语库、两家的 Key、回声策略、扬声器音量 |

字幕排版：**中文固定在左栏**，译文语言在右栏。灰色小字是原文，彩色大字是译文。
...
```
{ t:'ready', backend, pair, inputRate }
...
语言对是**建连参数**（`/ws?pair=zhja`），切换必须重连——中日要开两条上游会话。
